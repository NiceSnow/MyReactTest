...
export default class ScrollVertical extends Component {
  ...
  _clickNoticeTextView(urlString) {
      if(this.props.clickNoticeTextView){
        this.props.clickNoticeTextView(urlString)
      }
  }
  _createKbItem(kbItem, index) {
    return (
      <TouchableOpacity key={index}
                        onPress={() => this._clickNoticeTextView(kbItem.linkUrl)}
         style={[{justifyContent: 'center', height: this.state.scrollHeight}, this.props.scrollStyle]}>
        <GQCustomText style={[styles.kb_text_c, this.props.textStyle]}>{kbItem.title}</GQCustomText>
      </TouchableOpacity>
    )
  }
